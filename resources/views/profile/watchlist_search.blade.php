...
@section('content')
    <div class="flex flex-col my-20 w-full items-center">
        ...
        <form method="POST" action="{{ route('profile.watchlistCreate', $user->id) }}" class="w-5/6">
            @csrf
            <input id="search" name="search" type="text" value="{{ old('search') }}" class="flex w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md focus:border-blue-500 focus:outline-none focus:ring" placeholder="Search">
        </form>
    </div>
@endsection
