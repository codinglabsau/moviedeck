            ...
            <div class="container mx-auto flex px-5 py-16 md:flex-row flex-col items-start align-top">
                <div class="flex flex-col w-3/4 md:items-start md:text-left mr-10 mb-16 md:mb-0 items-center text-center bg-white p-12">
                    ...
                    @if(auth()->user()->is_admin && auth()->user() !=$user)
                        ...
                        @if($user->is_admin)
                            <h1 class="h-8 flex text-gray-600 items-center font-medium tracking-wide capitalize transition-colors duration-200 transform rounded-md">Already an Admin</h1>
                            <form method="post" action="{{ route('profile.removeAdmin', $user->id) }}">
                                @csrf
                                @method('PUT')
                                <button name="remove_admin" class="h-8 flex text-gray-600 items-center font-medium tracking-wide capitalize transition-colors duration-200 transform rounded-md border-2 border-gray-700 hover:border-gray-500">
