                ...
                <div class="container mx-auto flex px-5 py-32 mt-64 md:flex-row flex-col items-center align-baseline">
                    <div class="w-full lg:pr-24 md:pr-16 flex flex-col md:items-start md:text-left mb-16 md:mb-0 items-center text-center">
                        ...
                        <div x-data="{ open: false }" class="flex justify-center">
                            ...
                            @if(auth()->check())
                                <form method="post" action="{{ route('watchlist.store', ['user'=>auth()->user()->id, 'movie'=>$random->id]) }}">
                                    @csrf
                                    <button value="{{$random->id}}" name="movie_id" class="flex items-center ml-5 px-2 py-2 font-medium tracking-wide capitalize transition-colors duration-200 transform bg-transparent rounded-md hover:bg-gray-800 focus:outline-none focus:bg-blue-500">
                            ...
                            <div class="fixed top-0 left-0 flex items-center justify-center w-full h-full z-50 transition transition-all ease-in-out duration-1000" style="background-color: rgba(0,0,0,.8);" x-show="open">
                                <div class="h-auto p-4 mx-2 text-left bg-white rounded shadow-xl w-auto" @click.away="open = false">
                                    <iframe src="{{ $random->trailer }}" width="1280" height="720" frameborder="0" allowfullscreen></iframe>
                                </div>
                            </div>
                        </div>
                    </div>
